...
describe('FeatureToggles', () => {
  ...
  describe('isEnabled', () => {
    it('should return toggle value if testingSupport toggle exists', () => {
      const expectedToggleValue: boolean = toBoolean(config.get<boolean>(`featureToggles.dashboard`))
      expect(FeatureToggles.isEnabled('dashboard')).to.equal(expectedToggleValue)
    })
