...
function renderView (form: Form<Address>, res: express.Response): void {
  const defendantDetails = res.locals.user.legalClaimDraft.defendant.defendantDetails
...
export default express.Router()
  .get(Paths.defendantAddressPage.uri, (req: express.Request, res: express.Response) => {
    renderView(new Form(res.locals.user.legalClaimDraft.defendant.address), res)
  })
  .post(Paths.defendantAddressPage.uri, FormValidator.requestHandler(Address, Address.fromObject),
    ErrorHandling.apply(async (req: express.Request, res: express.Response, next: express.NextFunction): Promise<void> => {
      ...
      if (form.hasErrors()) {
        ...
        res.locals.user.legalClaimDraft.defendant.address = form.model
